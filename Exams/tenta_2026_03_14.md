...
**b)** Färdigställ funktionen `init_interrupts` i assemblerkoden nedan så att processorn använder mode `01` (vektorerade interrupts) och `mtvec` pekar på `vector_table`. (2.5p)
...
GPIO pinnar, PFIC och vektortabell är redan konfigurerade för att starta en interrupt handler när en extern interrupt sker på pin 14 eller 15 i port E.
