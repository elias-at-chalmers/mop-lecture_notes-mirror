 ...
 * Versions:
 * DBGCALL:	Calls are redirected through DBG307 builtin functions
 * CUSTOM: Calls directly implemented,  
 * <NO DEFINE>: Calls are implemented as ECALL traps..
*/
#ifdef DBGCALL
...
#ifdef WITH_LCD_PRINTF


// TODO	add custom vsprintf/strlen in this module 
void lcd_show_digit7(uint16_t x,uint16_t y,uint16_t fc, uint16_t bc, uint8_t num)
{  
extern const unsigned char font_7digit_mono[][50];	/* CHARACTER SIZE 11x25 */
	...
	if( islower( num) )
		num = toupper( num );
	num=num-' ';
	_tft_lcd_setwindow(x,y,x+10,y+24);
	for(pos=0;pos<50;pos+=2)
	{	/* Each row */
		temp=font_7digit_mono[num][pos];
		for(t=0;t<8;t++)
	    {  
	        if(temp&0x80)
				_lcd_write_16bit_data(bc); 
			else 
				_lcd_write_16bit_data(fc); 
			temp<<=1; 
		}
		temp=font_7digit_mono[num][pos+1];
		for(t=0;t<3;t++)
	    {  
	        if(temp&0x80)
				_lcd_write_16bit_data(bc); 
...
void lcd_show_ascii(int fontnum, uint16_t x,uint16_t y,uint16_t fc, uint16_t bc, uint8_t num)
{  
extern const unsigned char font_7digit_mono[][50];

    unsigned char temp1,temp2,pos;
	int width,heigth,limit,step,wbits, rigth_to_left=0; 
	switch( fontnum )
	{
		case FONT_ARIAL_NARROW_BOLD_24:
		num=num-' ';
			width = font_arial_narrow_bold_24.chars[num].char_width;
			heigth = font_arial_narrow_bold_24.chars[num].char_height;
			limit = font_arial_narrow_bold_24.chars[num].image->dataSize;
			break;
		default: return;
	}
		
	//_tft_lcd_setwindow(x,y,x+width-1,y+heigth-1);

	int bits = width*heigth;
	unsigned char *bytepos = font_arial_narrow_bold_24.chars[num].image->data;
...
static void lcd_puts_digit7(int fontnum, uint16_t x,uint16_t y,uint8_t size,uint8_t *p,uint8_t mode, uint16_t color, uint16_t background)
{  
	unsigned char t; 
	uint16_t width; 
	switch( fontnum )
	{
		case FONT_7DIGIT_11x25: width = 11; break;
		default:
		width = 8;	/*DEBUG */
	}
    while((*p<='~')&&(*p>=' '))
    {   
		if(x>(lcddev.width-1)||y>(lcddev.height-1)) 
			return;     
		lcd_show_digit7(x,y,color,background,*p);
        x+=width;
        p++;
    }  
} 

static void lcd_puts_ascii(int fontnum, uint16_t x,uint16_t y,uint8_t *p,uint8_t mode, uint16_t color, uint16_t background)
{  
	unsigned char t; 
	uint16_t width; 
	switch( fontnum )
    ...
    {   
		if(x>(lcddev.width-1)||y>(lcddev.height-1)) 
			return;     
		lcd_show_ascii(fontnum,x,y,color,background,*p);
        x+=width;
        p++;
    }  
} 
...
void
tft_printf (int fontnum, uint16_t x, uint16_t y, uint16_t bg, uint16_t fg, char * formatstr, ...)
{	/* Print text string with system font digital7-24 */
	char buf[256];
	va_list ap;
	va_start (ap, formatstr);

	if( fontnum > FONT_MAX )
		fontnum = FONT_DEFAULT;
	(void) vsprintf (buf, formatstr, ap);
	va_end(ap);
	if( fontnum == FONT_7DIGIT_11x25 )
		lcd_puts_digit7(fontnum, x,y,24,buf,0,fg, bg);
	else
		lcd_puts_ascii(fontnum, x ,y,buf,0, fg, bg);
}

#endif /* WITH_LCD_PRINTF */

#endif /* CUSTOM and <NO_DEFINE> */
