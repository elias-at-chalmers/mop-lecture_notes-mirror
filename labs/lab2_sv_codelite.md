...
Du *måste* ha gjort de förberedande uppgifterna ("Laboration 2 – förberedelse" på Canvas) för denna laboration. Om
du inte har gjort det kommer du troligtvis inte att hinna slutföra laborationsuppgiften i
tid.

## Laborationsmiljö

Vi fortsätter jobba med laborationsdatorn MD307, nu med
...
* Starta ett nytt workspace (eller öppna ett befintligt) i Codelite 
* Skapa ett nytt projekt (*Workspace -> New Project*.) Välj templaten `md307-startup`. 
* Öppna den skapade mappen `src` och ta bort filen `startup.c`
* Ladda ner och extrahera denna zip-fil med källkodsfiler
* Importera samtliga extraherade filerna genom att högerklicka på `src`-mappen och välj *Add an existing file...*

* Anslut *keypaden* till *Port D 15..8*. 
* Anslut *PTB-111* till *MD307* via
PTB-1000-bussen. 
* Anslut *ASCII-displayen* till *Port E* (*Data* till *15..8*,
*Control* till *7..0*).
...
Dags att köra programmet. Kompilera projektet med *Build → Build Project*
(<kbd>F7</kbd>). Starta sedan debuggern med *Debugger → Start/Continue Debugger*
(<kbd>F5</kbd>).
...
**Innan du går vidare:** Du måste vara *väldigt* säker på den visuella inspektion du gjort ovan.
...
Nu när du har modifierat koden bör du kompilera om projektet med
<kbd>F7</kbd>, köra programmet och kontrollera om det fungerar som avsett.
...
Sätt en brytpunkt vid anropet till `keyb` genom att klicka i marginalen till vänster om
raden i editorn (en röd punkt visas). Starta debuggern med <kbd>F5</kbd> och
tryck på *Continue* (<kbd>F5</kbd>) för att komma till brytpunkten. Håll ned en av knapparna
på *keypaden* och tryck *Continue* (<kbd>F5</kbd>). Kontrollera returvärdet i panelen
*Locals/Watches* längst ned. Är det vad du förväntade dig? Fortsätt
...
tryck *Continue* (<kbd>F5</kbd>). Uppfyller värdena för variablerna `row` och `col` i
*Locals*-panelen dina förväntningar? Svaret bör ge dig en ledtråd om buggen
ligger i `kbd_activate` eller `kbd_getcol`.
...
åtgärda och dokumentera eventuella buggar du stöter på. Du kan inspektera värdet
på `ctrl` i *Locals*-panelen.
