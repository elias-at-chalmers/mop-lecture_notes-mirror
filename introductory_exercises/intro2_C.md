...
* Tryck Ctrl+Shift+P (Command+Shift+P på Mac) och skriv sedan `MDx07: Initialize Project` i textrutan. Tryck return.
* Välj sedan MD307 C Project som template och tryck return.

Precis som när vi startade ett assemblerprojekt så skapas nu ett gäng filer i katalogen:
...
**Starta simulatorn**, precis som när du tidigare arbetat med assembler. 
Knacka sedan in den här koden i main.c:
![](../images/intro2/Picture2.png)

Titta på ditt program och se till att du förstår vad det borde göra (återvänd annars till lektionsanteckningarna). Sätt sedan en brytpunkt (`F9`) på rad 7 och tryck på `F5` för att kompilera och köra programmet.
...
Ni kan nu starta programmet igen (`F5`) och stega er igenom det för att se att det fungerar. Om ni väljer "Step Into" när programmet är på rad 6 eller 7 kommer debuggern att låta er stega igenom assemblerkoden. 
